...
void effectMeteorShower(EffectContext &ctx) {
  ...
  for (int m = 0; m < METEOR_COUNT; m++) {
    ...
    float head =
        effect_common::fmodf_positive(t * speed + phase * cycle, cycle);
    uint8_t hr, hg, hb;
    ...
    for (float ti = 0; ti <= tailLen; ti += 1.0f) {
      ...
      ctx.strip->setPixelColor((uint16_t)idx, effect_common::scale8(hr, bright),
                               effect_common::scale8(hg, bright),
                               effect_common::scale8(hb, bright));
    }
  }
  ctx.strip->show();
}
