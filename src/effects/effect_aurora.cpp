...
void effectAurora(EffectContext& ctx) {
  ...
  for (uint16_t i = 0; i < n; i++) {
    ...
    if (wave < 0) wave = 0;
    if (wave > 1) wave = 1;
    float hueFloat = effect_common::fmodf_positive(y * 80.0f + t * 30.0f, 256.0f);
    uint8_t hue = (uint8_t)hueFloat;
    uint8_t r, g, b;
    effect_common::hueToRgb(hue, r, g, b);
    uint8_t bright = (uint8_t)(wave * 200.0f);
    ctx.strip->setPixelColor(i, effect_common::scale8(r, bright), effect_common::scale8(g, bright), effect_common::scale8(b, bright));
  }
  ctx.strip->show();
}
